...
describe('POST /api/users request tests', () => {
  test('a new user is created', async () => {
    ...
    await api.post('/api/users').send(helper.individualUser).expect(201)

    const usersInDbAfterPostRequest = await helper.getUsersInDb()
    const usersInDbAfterPostRequestString = JSON.stringify(
      usersInDbAfterPostRequest
    )

    expect(usersInDbAfterPostRequest).toHaveLength(
      usersInDbBeforePostRequest.length + 1
    )
    expect(usersInDbAfterPostRequestString).toContain(
      helper.individualUser.username
    )
    expect(usersInDbAfterPostRequestString).toContain(
      helper.individualUser.name
    )
    expect(usersInDbAfterPostRequestString).not.toContain(
      helper.individualUser.password
    )
  })
  ...
  test('when no username is given respond with 400 and does not create new user', async () => {
    ...
    expect(usersInDbAfterPostRequest).toHaveLength(
      usersInDbBeforePostRequest.length
    )
    expect(usersInDbAfterPostRequestString).not.toContain(
      userWithoutUsername.name
    )
  ...
  test('when username is 3 chars respond with 201 and create new user', async () => {
    ...
    await api.post('/api/users').send(userWithUsernameOfThreeChars).expect(201)
    ...
    expect(usersInDbAfterPostRequestString).toContain(
      userWithUsernameOfThreeChars.name
    )
    expect(usersInDbAfterPostRequestString).toContain(
      userWithUsernameOfThreeChars.username
    )
  })
  ...
  test('when given password 3 characters long respond with 201', async () => {
    ...
    await api.post('/api/users').send(userWithTooShortPassword).expect(201)
